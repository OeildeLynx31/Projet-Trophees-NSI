...
def initFile(fileName, headers=[], data=[]):
    if not fileExist(fileName):
        with open('./data/storage/'+fileName+'.csv', 'w', newline='') as csvfile:
...
def fileExist(fileName):
    return os.path.isfile('./data/storage/' + fileName + '.csv')

def readFile(fileName):
    if (fileExist(fileName)):
        with open('./data/storage/'+fileName+'.csv', newline='') as csvfile:
...
def writeFile(fileName, data):
    if (fileExist(fileName)):
        ...
        try:
            with open('./data/storage/'+fileName+'.csv', 'w', encoding="utf-8", newline='') as csvfile:
...
def getColumns(fileName):
    if (fileExist(fileName)):
        with open('./data/storage/'+fileName+'.csv') as csvfile:
